...
const QuoteForm = ({
  ...
  const initialProjectDescription =
    serviceName && typeof serviceName === "string"
      ? `Quote request for: ${decodeURIComponent(serviceName)}`
      : "";
  ...
  const handleSubmit = async (e) => {
    ...
    // Create FormData for file upload (include serviceId and serviceName when from service detail)
    ...
    try {
      ...
      // Reset form (keep service context if from service detail)
      setFormData({
        name: "",
        company: "",
        phone: "",
  ...
  return (
    ...
    >
      {/* Service context when coming from service detail */}
      {displayServiceName && (
        <div
          style={{
            background: "rgba(0, 51, 150, 0.1)",
            border: "1px solid rgba(0, 51, 150, 0.3)",
            color: "#003396",
            padding: "12px 20px",
            borderRadius: "8px",
            marginBottom: "24px",
        ...
        >
          Requesting quote for: <strong>{displayServiceName}</strong>
        </div>
      )}
      {/* Tag/Badge */}
      <div
